...
import { upsertUserByPhone, markPhoneVerified, ensureWorkerProfile, setWorkerUpiId, updateWorkerAvailabilityInDb, createJob, insertJobEvent, setJobStatus, createOffers, listWorkerPendingOffers, decideOffer, assignWorkerToJob, getJobWithPhones, setWorkerApproval, createFeeRow } from "./db/queries.js";
import { requireAuth, requireRole } from "./auth/mvpAuth.js";
...
// --- Redis keys ---
function workerLocKey(workerId) {
    return `workerloc:${workerId}`;
}
function cellWorkersKey(cellKey) {
...
app.post("/auth/otp/request", async (req, res) => {
    ...
    if (role === "worker")
        await ensureWorkerProfile(db, userId);
    const otp = generateOtpCode();
    const hash = otpHash(otp);
    const expiresAt = new Date(Date.now() + cfg.otpExpiresSeconds * 1000);
    const q = await db.query(`insert into otp_requests(phone, role, otp_hash, expires_at, attempts_used, is_verified)
     values ($1,$2,$3,$4,0,false)
     returning id`, [phone, role, hash, expiresAt.toISOString()]);
...
app.post("/auth/otp/verify", async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: "invalid_body", details: parsed.error.flatten() });
    const row = await db.query(`select id, phone, otp_hash, expires_at, attempts_used, is_verified
     from otp_requests where id=$1`, [parsed.data.requestId]);
    if (row.rowCount !== 1)
    ...
    if (!verify.ok)
        return res.status(400).json({ error: verify.reason ?? "otp_failed" });
    ...
    if (u.rowCount === 1)
        await markPhoneVerified(db, u.rows[0].id);
    // MVP token is userId
    return res.json({ ok: true, token: u.rows[0].id });
});
// --- Worker availability + location index ---
app.post("/workers/availability", requireAuth(db), requireRole("worker"), async (req, res) => {
    ...
    if (workerUpiId)
        await setWorkerUpiId(db, workerId, workerUpiId);
...
// --- Jobs + dispatch ---
app.post("/jobs", requireAuth(db), requireRole("client"), async (req, res) => {
    const schema = z.object({
...
app.get("/jobs/:jobId", requireAuth(db), async (req, res) => {
...
app.get("/workers/me/offers", requireAuth(db), requireRole("worker"), async (req, res) => {
    const offers = await listWorkerPendingOffers(db, req.user.id);
    return res.json({ offers });
});
// Worker: accept/reject offer
app.post("/job_offers/:offerId/decision", requireAuth(db), requireRole("worker"), async (req, res) => {
    ...
    if (parsed.data.decision === "Accepted") {
        ...
        if (!assigned)
            return res.status(409).json({ error: "job_already_assigned" });
...
app.post("/jobs/:jobId/status", requireAuth(db), async (req, res) => {
    const schema = z.object({
    ...
    if (parsed.data.status === "Completed" && !isClient)
        return res.status(403).json({ error: "only_client_can_complete" });
    if ((parsed.data.status === "Arrived" || parsed.data.status === "Started") && !isWorker) {
        return res.status(403).json({ error: "only_worker_can_update" });
    }
    await setJobStatus(db, jobId, parsed.data.status);
...
app.post("/admin/workers/:workerId/approve", requireAuth(db), requireRole("admin"), async (req, res) => {
    await setWorkerApproval(db, req.params.workerId, true);
    return res.json({ ok: true });
});
app.get("/admin/jobs/:jobId/events", requireAuth(db), requireRole("admin"), async (req, res) => {
...
app.post("/admin/jobs/:jobId/cancel", requireAuth(db), requireRole("admin"), async (req, res) => {
