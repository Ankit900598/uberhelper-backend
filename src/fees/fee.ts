export interface CreateFeeInput {
  jobAmountCashCents: number; // total cash client will pay worker
  feePercent?: number; // default 10.00 (%)
}

export function calcPlatformFeeCents(input: CreateFeeInput): number {
...
export interface UpilinkInput {
  payeeUpiId: string; // you/your platform UPI ID (PA)
  payeeName: string; // optional display name
  amountCents: number;
  currency?: "INR";
  transactionRef: string; // unique per fee payment
  note?: string;
}
...
export function generateUpiPayLink(input: UpilinkInput): string {
